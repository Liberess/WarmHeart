...
public class Manpower : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Player")
        {
            ...
            player.gameObject.GetComponent<Rigidbody2D>().AddForce(Vector2.up * y_direction * Power, ForceMode2D.Impulse);
            player.gameObject.GetComponent<Rigidbody2D>().AddForce(Vector2.right * x_direction * Power, ForceMode2D.Impulse);
            Invoke("delete",1f);
        }
    }
    void delete()
